...
public class StockAccount extends Account {
    ...
    StockAccount(int id, Currency currency) {
        ...
        if (CustomerSearchingFunction.searchStockMoneyAmount(customerID) != null) {
            stockBalance = CustomerSearchingFunction.searchStockMoneyAmount(
                    customerID);
        }
        else {
            stockBalance = null;
    ...
    public void setStockBalance(BigDecimal s) {
        this.stockBalance = s;
    }
...
//1000
    public boolean initAccount() {
        System.out.println(
                "Welcome! You will start your trip in the stock market!.");
        System.out.println(
                "Opening this count will charge you 5 dollars." + '\n'
        ...
        if (currency.get("Dollar").compareTo(new BigDecimal("5000")) < 0) {
            ...
            if (success) {
                while (true) {
                    ...
                    if (cash[0].equals("")) {
                        System.out.println(
                                "You should save more than 1000 dollars to initialize this account!");
                    }
                    else {
                        //alter manager
                        ...
                        createTransaction(cap.toString(), "Dollar",
                ...
                System.out.println(
                        "Fail to open a stock account. Pay your money back.");
                createTransaction("0", "Dollar", "Failed to open stock account.");
                return false;
            }
        }
    ...
    public void Menu() {
        ...
        switch (number) {
            case 0:
                System.out.println(
                        "You have to save over 1000 dollars, and you must maintain a $2500.00 balance in your saving account.");
                boolean flag = true;
                ...
                while (flag) {
                    ...
                    if (!Tool.is_number(saveMoney)) {
                        ...
                        if (stockMoney.compareTo(LEASTMONEY) != 1) {
                            System.out.println(
                                    "You have to save over 1000 dollars one time!Try again");
                        }
                        else {
                            if (currency.get("Dollar").compareTo(stockMoney.add(
                                    LEASTBALANCE)) == -1) {
                                System.out.println(
                                        "You have to have at least 2500 dollars balance in saving account!");
                                flag = false;
                            }
                            else {
                                boolean success = currency.sub("Dollar",
                                                               stockMoney.doubleValue(),
                                                               "1");
                                if (success) {
                                    createTransaction(
                                            stockMoney.multiply(TRANS).toString(),
                                            "Dollar",
                                            "Transfer money to stock account.");
                                    CustomerAlteringFunction.alterSavingAccount(
                                            customerID, "Dollar",
                                            stockMoney.multiply(TRANS));
                                    if (this.stockBalance == null) {
                                        this.stockBalance = stockMoney;
                                        CustomerAlteringFunction.alterStockAccount(
                                                customerID, stockMoney);
                                    }
                                    else {
                                        this.stockBalance = this.stockBalance.add(
                                                stockMoney);
                                        CustomerAlteringFunction.alterStockAccount(
                                                customerID, stockMoney);
                                    }
            ...
            case 1:
                if (CustomerSearchingFunction.searchStockList(customerID) != null) {
                    List<String[]> stockList = CustomerSearchingFunction.searchStockList(
                            customerID);
            ...
            case 2:
                ...
                System.out.println(
                        "What's the stock and what's the number you want to buy?");
                ...
                while (flag2) {
                    ...
                    if (stockNum <= 0) {
                        ...
                        if (stockBalance != null && payMoney.compareTo(
                                stockBalance) > 0) {
                            ...
                            flag2 = false;
                            this.stockBalance = this.stockBalance.subtract(
                                    payMoney);
                            createTransaction("-" + payMoney.toString(),
                                              "Dollar", "Buy stocks.");
                            CustomerAlteringFunction.alterStockAccount(
                                    customerID, payMoney.multiply(TRANS));
                            CustomerAddingFunction.addStockOwnership(customerID,
                                                                     stockId,
            ...
            case 3:
                ...
                if (cusStocklist != null) {
                    ...
                    CustomerDeletingFunction.deleteOneStock(customerID,
                                                            Integer.parseInt(
                                                                    cusStocklist.get(
                                                                            index1)[0]));
                    ...
                    createTransaction(profit.toString(), "Dollar",
                                      "Sell stocks.");
                }
                else {
                    System.out.println("You don't have stocks.");
