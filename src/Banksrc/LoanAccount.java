...
public class LoanAccount extends Account {
    ...
    public boolean initAccount() {
        System.out.println(
                "Welcome! You will be applying loans and pay for collterals here, starting today.");
        System.out.println(
                "Opening this count will charge you 8 dollars. Make sure you have created the saving count and save at least 8 dollars in it.");
    ...
    public void Menu() {
        System.out.println(
                "1. apply for a loan 2. check loans 3. pay for loans 4. Exit");
    ...
    public void payLoan() {
        System.out.println(
                "Please enter the name of the collteral you wanna redeem.");
        ...
        if (id >= 0) {
            ...
            boolean success = Account.currency.sub(
                    collaterals.get(id).getCurrencyType(), Double.parseDouble(
                    price), "1");
            if (success) {
                System.out.println(
                        "Now you have your " + collaterals.get(id).getItem() + " again!");
                System.out.println("Thank you for using our bank！");
                createTransaction("-" + collaterals.get(id).getPrice(),
                                  collaterals.get(id).getCurrencyType(),
                                  "Pay for the chosen collateral. " + collaterals.get(
                                          id));
                collaterals.remove(id);
            }
            else {
                createTransaction("0", collaterals.get(id).getCurrencyType(),
                                  "Failed to pay for the chosen collateral. " + collaterals.get(
                                          id));
            }
        }
    }
    ...
    public void applyLoan() {
        System.out.println(
                "Input the name and price you have assessed here, we will loan you 80% of your collateral.");
        ...
        if (collaterals.size() > 0) {
            while (alreadyExist) {
                for (int i = 0; i < collaterals.size(); i++) {
                    if (collaterals.get(i).getItem().equals(name)) {
                        System.out.println(
                                "You cannot have items with the same name, please input again.");
                        System.out.println(
                                "For example, you already pledged a 'car'. You want to pledge a car now, you can name it 'car1'.");
                        name = colla.nextLine();
                        break;
                    }
                    alreadyExist = false;
        ...
        switch (number) {
            case 1: {
                System.out.println(
                        "Please make sure that your collateral is over $250 or we won't make a loan for you.");
                ...
                if (priceD < 250) {
                    System.out.println(
                            "Your item is too cheap and cannot be a collateral in our bank, sorry.");
                    createTransaction("0", "Dollar",
                                      "Failed to loan cause the collateral is too cheap.");
                }
                else {
                    System.out.println(
                            "Ok! We will loan you 90% of this collateral.");
                    Account.currency.add("Dollar", priceD, loanRate);
                    Collateral collateral = new Collateral(name, item,
                                                           "Dollar");
                    collaterals.add(collateral);
                    BigDecimal addNum = new BigDecimal(Double.toString(
                            priceD));
                    addNum = addNum.multiply(new BigDecimal(loanRate));
                    String money = addNum.toString();
                    createTransaction(money, "Dollar",
                                      "Apply for a loan. " + collateral);
                }
            }
            case 2: {
                System.out.println(
                        "Please make sure that your collateral is over ¥2000 or we won't make a loan for you.");
                System.out.println("The price of your collateral(¥):");
                item = colla.nextLine();
                while (!Tool.is_number(item)) {
                    System.out.println("Invalid input. Input again.");
                ...
                if (priceR < 2000) {
                    ...
                    addNum = addNum.multiply(new BigDecimal(loanRate));
                    String money = addNum.toString();
                    createTransaction(money, "RMB",
                                      "Apply for a loan. " + collateral);
                }
            }
            case 3: {
                System.out.println(
                        "Please make sure that your collateral is over ￡200 or we won't make a loan for you.");
                System.out.println("The price of your collateral(￡):");
                item = colla.nextLine();
                while (!Tool.is_number(item)) {
                    System.out.println("Invalid input. Input again.");
                ...
                if (price < 200) {
                    ...
                    Collateral collateral = new Collateral(name, item,
                                                           "Pound");
                    collaterals.add(collateral);
                    BigDecimal addNum = new BigDecimal(
                            Double.toString(price));
                    addNum = addNum.multiply(new BigDecimal(loanRate));
                    String money = addNum.toString();
                    createTransaction(money, "Pound",
                                      "Apply for a loan. " + collateral);
                }
            }
        }
